...
* ```main.Point``` isimli bir sınıf tanımlayınız.
...
* main.Point sınıfı için bir adet ```constructor``` tanımlayınız. İki değeri de set edebilmeli.
* İlgili iki değişken için getter ve setter metodlarını tanımlayınız.
* main.Point sınıfı içerisinde bir tane ```distance``` isminde metod tanımlayınız.
...
*  ```distance(main.Point p)``` şeklinde başka bir main.Point objesi alırsa verilen noktaya göre uzaklık hesaplanmalı.
...
INPUT
main.Point first = new main.Point(6, 5);

main.Point second = new main.Point(3, 1);
...
main.Point point = new main.Point(0,0);
